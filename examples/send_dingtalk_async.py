# 作者：Xiaoqiang
# 微信公众号：XiaoqiangClub
# 创建时间：2025-11-12T00:13:25.068Z
# 文件描述：异步发送钉钉消息的示例
# 文件路径：examples/send_dingtalk_async.py

import asyncio
from xqcsendmessage import send_dingtalk_async  # 导入新的顶层函数

# --- 配置 ---
# 你的钉钉机器人的 Webhook 和密钥
# 请替换为您的钉钉机器人 Webhook URL
WEBHOOK_URL = "YOUR_DINGTALK_WEBHOOK_URL"
# 请替换为您的钉钉机器人密钥，如果没有则为 None
...
async def main():
    ...
    try:
        ...
        result_direct_str = await send_dingtalk_async(
            "这是一条来自 sendmessage 直接发送的异步测试消息 (字符串)。",
            webhook=WEBHOOK_URL,
            secret=SECRET,
        ...
        result_direct_str_at = await send_dingtalk_async(
            "这是一条来自 sendmessage 直接发送的异步测试消息 (覆盖 @ 人，指定手机号，markdown 类型)。",
            webhook=WEBHOOK_URL,
            secret=SECRET,
            msg_type="markdown",
        ...
        result_direct_dict = await send_dingtalk_async(
            {
                ...
                "markdown": {
                    "title": "测试标题",
                    "text": "### 【XQCSendMessage】这是一条通过 sendmessage 直接发送的异步测试消息 (字典，指定手机号，markdown 类型)。"
                }
            },
            webhook=WEBHOOK_URL,
            secret=SECRET,
