# 作者：Xiaoqiang
# 微信公众号：XiaoqiangClub
# 创建时间：2025-11-12T00:13:34.573Z
# 文件描述：同步发送企业微信 Webhook 消息的示例
# 文件路径：examples/send_wecom_webhook_sync.py

from xqcsendmessage import send_wecom_webhook  # 导入新的顶层函数
...
def main():
    ...
    try:
        ...
        result_direct_str = send_wecom_webhook(
            "【XQCSendMessage】这是一条来自 sendmessage 直接发送的同步 Webhook 测试消息 (字符串)。",
            webhook=WEBHOOK_URL,
        ...
        result_override_str = send_wecom_webhook(
            {"markdown": {"content": "【XQCSendMessage】这是一条来自 sendmessage 的同步 Webhook 测试消息 (覆盖字符串内容，markdown 类型)。"}},
            webhook=WEBHOOK_URL,
        ...
        result_direct_dict = send_wecom_webhook(
            {
                "msgtype": "markdown",
                "markdown": {"content": "【XQCSendMessage】这是一条来自 sendmessage 直接发送的同步 Webhook 测试消息 (字典，markdown 类型)。"}
            },
            webhook=WEBHOOK_URL,
