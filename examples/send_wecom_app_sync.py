# 作者：Xiaoqiang
# 微信公众号：XiaoqiangClub
# 创建时间：2025-11-12T00:13:54.855Z
# 文件描述：同步发送企业微信应用消息的示例
# 文件路径：examples/send_wecom_app_sync.py

from xqcsendmessage import send_wecom_app  # 导入新的顶层函数
...
def main():
    ...
    try:
        ...
        result_direct_str = send_wecom_app(
            "【XQCSendMessage】这是一条通过企业微信应用直接发送的同步测试消息 (字符串)。",
            corpid=CORP_ID,
        ...
        result_direct_dict = send_wecom_app(
            {
                "msgtype": "markdown",
                "markdown": {"content": "【XQCSendMessage】这是一条通过企业微信应用直接发送的同步测试消息 (字典，指定用户)。"}
            },
            corpid=CORP_ID,
            corpsecret=CORP_SECRET,
            agentid=AGENT_ID,
